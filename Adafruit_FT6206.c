...
*/
/**************************************************************************/
uint8_t Adafruit_FT6206_setup(uint8_t threshhold) {
...
void Adafruit_FT6206_readData(uint16_t *x, uint16_t *y, uint16_t *z) {

    uint8_t i2cdat[16];
    uint8_t data = 0;
    if (bspI2cSelect(0,FT6206_ADDR)) {
        bspI2cWrite(&data,1);
  ...
  */
    ...
    */
    ...
    Serial.print("Weight: "); Serial.print(touchWeight[i]); Serial.print("\t("); Serial.print(touchX[i]);
...
TS_Point Adafruit_FT6206_getPoint(void) {
  uint16_t x, y, z;
  //uint8_t z;
  Adafruit_FT6206_readData(&x, &y, &z);
  return TS_Point_Init(x, y, z);
}
...
TS_Point* TS_Point_New(int16_t x0, int16_t y0, int16_t z0) {
  TS_Point* point = (TS_Point*) malloc(sizeof(TS_Point));
  point->x = x0;
  point->y = y0;
  point->z = z0;
  return point;
